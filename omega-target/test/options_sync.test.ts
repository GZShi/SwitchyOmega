...
describe("OptionsSync", () => {
  ...
  describe("#requestPush", () => {
    ...
    it("should schedule storage write", (done) => {
      const check = () => {
        if (
          (storage.set as any).callCount === 0 ||
          (storage.remove as any).callCount === 0
        )
          return;
        (storage.set as any).should.have.been.calledOnce.and.calledWith({
          b: 1,
        });
        (storage.remove as any).should.have.been.calledOnce.and.calledWith([
          "a",
        ]);
        done();
      };
