...
console.log('=== Step 1: Compile individual CoffeeScript files ===');
try {
  execSync(
    `"${path.join(root, 'node_modules', '.bin', 'coffee')}" -c -o "${path.join(buildDir, 'js')}" "${path.join(root, 'src', 'coffee')}"`,
    { cwd: root, stdio: 'inherit' }
  );
} catch (e) {
  // coffee CLI may return non-zero even on success for warnings
  console.log('CoffeeScript compilation finished.');
}
...
const omegaTargetMin = path.join(root, 'node_modules', 'omega-target', 'omega_target.min.js');
...
console.log('=== Step 5: Copy overlay files (manifest, background.html, etc.) ===');
